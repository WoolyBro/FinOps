...
def diagnose_failure(exc: BaseException, model_id: str, region: str | None) -> str:
    ...
    if "AccessDenied" in name or "AccessDenied" in text:
        return (
            ...
            f"  region. Bedrock console > Model access > enable {model_id}.\n"
            "  Model access is granted per region -- enabling it elsewhere does\n"
            "  not help. First-party AWS models (Amazon Nova) need only that\n"
            "  one click; third-party models (Anthropic, Meta, Mistral) also\n"
            "  require accepting a usage-terms agreement before authorization\n"
            "  status moves off NOT_AUTHORIZED -- check both if this model is\n"
            "  from a third party.\n"
